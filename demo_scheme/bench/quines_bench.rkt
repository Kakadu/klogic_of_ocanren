...
(define results
  (run-benchmarks
   ...
   (list 'quines100
         'twines15
         'thrines2
         ;'quines100_syn 'twines15_syn 'thrines2_syn
         ; 'sleepHalf
         )
        ...
        (myrun 2
               (lambda (p)
                 (fresh (q r s)
                        ...
                        (eval-expo s '() `(val_ ,q)))))]
       ;
       ['quines100_syn (myrun 100 (lambda (q) (evalo2 q '() `(val_ ,q))))]
       ['twines15_syn
        (myrun 15
               (lambda (p)
                 (fresh (q r) (== p `(,q ,r)) (evalo2 q '() `(val_ ,r)) (evalo2 r '() `(val_ ,q)))))]
       ['thrines2_syn
        (myrun 2
               (lambda (p)
                 (fresh (q r s)
   ...
   #:num-trials 1 ; TODO: 40 is better
